...
> = {
  Completed: { color: "green", icon: CheckCircle, label: "Completada" },
  Pending: { color: "yellow", icon: Clock, label: "Pendiente" },
  "In Process": { color: "blue", icon: RefreshCw, label: "En curso" },
  Cancelled: { color: "red", icon: Clock, label: "Cancelada" },
};

const NEXT_STATUSES: Record<string, AppointmentStatus[]> = {
  Pending: [AppointmentStatus.InProcess, AppointmentStatus.Cancelled],
  "In Process": [AppointmentStatus.Completed, AppointmentStatus.Cancelled],
  Completed: [],
  Cancelled: [],
};
      ...
      <Stack gap={6}>
        ...
        <Group gap="xs">
          ...
          <Text size="sm" c="dimmed">
            {isProvider
              ? `Cliente: ${appointment.client_name ?? `#${appointment.user_id}`}`
              : `Proveedor: ${appointment.provider_name ?? `#${appointment.provider_id}`}`}
          </Text>
        </Group>
        ...
        <Group gap="xs">
          ...
          {appointment.travel_buffer_min > 0 && (
            <Text size="xs" c="dimmed">
              (+{appointment.travel_buffer_min} min desplazamiento)
            </Text>
          )}
        </Group>
        ...
        <Group justify="space-between">
          <Group gap="xs">
            ...
            {isProvider && (
              <Text size="xs" c="dimmed">
                (neto: €{appointment.provider_net.toFixed(2)})
              </Text>
            )}
          </Group>
        ...
        {isProvider && nextStatuses.length > 0 && (
          <Group gap="xs" mt={4}>
            {nextStatuses.map((s) => (
              ...
              >
                Marcar como {statusConfig[s]?.label ?? s}
              </Button>
            ))}
          </Group>
        )}
            ...
            <ScrollArea mah={500}>
              <Stack gap="sm">
                {appointmentsOnSelectedDate.length > 0 ? (
                  ...
                  <Center py={50}>
                    <Text c="dimmed">No hay eventos para este día</Text>
                  </Center>
                )}
              </Stack>
            </ScrollArea>
