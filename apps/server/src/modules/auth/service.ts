...
export abstract class AuthService {
    static async register({
        ...
        if (existing) {
            throw status(400, 'Email already in use' satisfies AuthModel['registerInvalid']);
        }
    ...
    static async login({
        ...
        if (!existing) {
            throw status(400, 'User not exists' satisfies AuthModel['loginUserNotExists']);
        }

        const firebaseToken = await firebaseAuth.createCustomToken(existing.name, {
