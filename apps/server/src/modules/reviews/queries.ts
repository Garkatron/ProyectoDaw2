...
export const ReviewsQueries = {
    ...
    findByReviewedId: db.query<Review & { reviewer_name: string }, { reviewed_id: number }>(
        `SELECT r.*, u.name AS reviewer_name 
         FROM Reviews r
         JOIN Users u ON r.reviewer_id = u.id
    ...
    findExistingByAppointment: db.query<{ id: number }, { reviewer_id: number; appointment_id: number }>(
    ...
    getAll: db.query<Review & { reviewer_name: string; reviewed_name: string }, null>(
    ...
    getAverageRating: db.query<{ avg_rating: number; total_reviews: number }, { reviewed_id: number }>(
